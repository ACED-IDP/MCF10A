...
EMITTERS = {}
ALREADY_EMITTED = set()
RAW_DATA_PATH = pathlib.Path('data/raw/sample_annotations.ndjson')
FHIR_DATA_PATH = pathlib.Path('data/fhir')
FHIR_SPECIMEN_PATH = pathlib.Path('data/fhir/Pre_Transform/Specimen.ndjson')
FHIR_TASK_PATH = pathlib.Path('data/fhir/Pre_Transform/Task.ndjson')
...
def emit_specimen(output_path, specimen_line, annotation_line):
...
@click.argument('file_path', default=RAW_DATA_PATH)
@click.argument('output_path', default=FHIR_DATA_PATH)
@click.argument('specimen_path', default=FHIR_SPECIMEN_PATH)
@click.argument('task_path', default=FHIR_TASK_PATH)
@click.option('--project_id', required=True,
              default='aced-nvidiaCollab',
              show_default=True,
              help='program-project'
              )
def transform_directory(file_path, output_path,specimen_path, task_path, project_id):
	...
	with open(file_path, "rt") as fp, open(specimen_path, "rt") as sp, open(task_path, "rt") as tp:
		...
		for annotation_line, specimen_line in zip(annotation_lines,specimen_lines):
			...
			emit_specimen(output_path, specimen_line,annotation_line)
